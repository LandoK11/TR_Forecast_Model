...
input_data['year'] = input_data.apply(lambda x: str(x['registration_expiration_date'].year), axis=1)
...
input_data = input_data.drop(columns=['registration_expiration_date', 'Count'])
...
##### PIPELINE ######
cat_columns = ['Make', 'Customer', 'CountyId', 'IF_SEEN_BEFORE']
# num_columns = ['VehicleYear', 'Count', 'month', 'year']
num_columns = ['VehicleYear', 'month', 'year']
